...
class Bot:
    def __init__(self, user_id):
        ...
        self.next_command = "talk" # Переменная необходимая, чтобы переключать режим работы бота
        self.current_quesion = 0

    # Принимает команду пользоваетеля и обрабатывает её. В качестве ответа отправляет класс Answer
    def update(self, text):
        # Передается входное значение в функцию
        if self.next_command == "talk":
            return self.talk(text)

        if self.next_command == "welcome":
            return self.welcome()

        if self.next_command == "test":
            return self.test(text)

        if self.next_command == "show_meme":
            return self.show_meme(text)

        if self.next_command == "show_user_stat":
            return self.show_user_stat(text)

        if self.next_command == "show_all_stat":
            return self.show_all_stat(text)

        if self.next_command == "upload_meme":
